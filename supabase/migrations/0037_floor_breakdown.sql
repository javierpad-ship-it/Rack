-- ============================================================================
-- 0037 — Detalle de Piso vs Almacén dentro de una tienda, por dimensión
--
-- Para "entrar" a una tienda y ver la realidad por Responsable / Género /
-- Mundo / Línea SAP / Código de artículo: rotación (IRP) y DÓNDE está la
-- mercadería (piso escaneado vs almacén = stock − piso).
--   p_by ∈ ('resp','gender','mundo','linea','articulo')
-- ============================================================================
...
begin
  ...
  select max(snapshot_date) into v_snap from stock_snapshots where store_id = p_store;

  with dims as (
    select sku, max(resp) resp, max(gender) gender, max(mundo) mundo, max(sap_line) linea,
           max(article_code) articulo, sum(units)::numeric stk
    from stock_snapshots where store_id = p_store and snapshot_date = v_snap group by sku
  ),
  piso as (
