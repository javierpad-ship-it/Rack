-- ============================================================================
-- 0034 — IRP proyectado en los mapas de calor (precio y talla)
--
-- Los heatmaps de rotation_report (por grupo) y rotation_stores (por tienda)
-- ahora incluyen irp_proy por celda: ventas × factor / (ventas × factor + stock),
-- con el mismo factor de regla de 3 por días transcurridos que el resto del
-- reporte. La web permite alternar entre IRP actual y proyectado.
-- ============================================================================
...
begin
  ...
  select json_build_object(
    ...
    'price_hm', (
      select coalesce(json_agg(json_build_object('grp', grp, 'band', band, 'irp', irp, 'irp_proy', irp_proy, 'cant', cant, 'stk', stk)), '[]')
      from g_price
    ),
    'talla_hm', (
      select coalesce(json_agg(json_build_object('grp', grp, 'talla', talla, 'irp', irp, 'irp_proy', irp_proy, 'cant', cant, 'stk', stk)), '[]')
      from g_talla
    )
  ) into v_result;
...
begin
  ...
  select json_build_object(
    ...
    'price_hm', (
      select coalesce(json_agg(json_build_object('grp', store, 'band', band, 'irp', irp, 'irp_proy', irp_proy, 'cant', cant, 'stk', stk)), '[]')
      from s_price
    ),
    'talla_hm', (
      select coalesce(json_agg(json_build_object('grp', store, 'talla', talla, 'irp', irp, 'irp_proy', irp_proy, 'cant', cant, 'stk', stk)), '[]')
      from s_talla
    )
  ) into v_result;
