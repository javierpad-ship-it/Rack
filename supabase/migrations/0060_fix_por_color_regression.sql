-- ============================================================================
-- 0054 — Fix: detalle del genérico por color volvía vacío
--
-- Otra migración concurrente reescribió prisma_variant_lookup() (agregando el
-- concepto de "empresa" — tabla `empresas`, `stores.empresa_id`,
-- `generic_price_info_empresa()`, que se conserva íntegro aquí) y, al hacerlo,
-- revirtió el fix de 0051: volvió a filtrar el CTE `gen` por
-- `sc.generic_code is not distinct from v_generic` en vez de
-- `pv_generic(sc.sku) is not distinct from v_generic`. Como
-- `stock_current.generic_code` está NULL en casi todas las filas (nunca se
-- pobló por import — el genérico se deriva del SKU, ver 0051), el filtro no
-- matcheaba nada y `por_color` quedaba en `[]` para cualquier producto.
--
-- Se conserva TODO lo agregado por esa migración concurrente (empresa_id,
-- empresa_nombre, generic_price_info_empresa) — solo se corrige el filtro
-- del CTE `gen` para volver a usar el genérico derivado.
-- ============================================================================
