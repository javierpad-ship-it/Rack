-- ============================================================================
-- 0050 — Prisma: RPCs de consulta
--
-- `prisma_variant_lookup`  → ficha del producto al escanear una variante.
-- `price_proposals_review` → bandeja del responsable de línea (con stock y
--                            rotación a nivel tienda y cadena).
--
-- Rotación = IRP de Rack One = ventas(und)/(ventas+stock)×100 (ver
-- 0024_rotation_report.sql). Piso vs almacén se reutiliza de store_warehouse()
-- (0002). Todo por CODIGO_GENERICO (Material) resuelto vía generic_of_sku().
-- ============================================================================

-- ---------- Ficha del producto (escaneo) ----------
...
-- ---------- Bandeja de propuestas (responsable de línea) ----------
-- Devuelve, por propuesta visible al usuario, los datos de decisión + stock y
-- rotación (IRP) a nivel TIENDA (la del solicitante) y CADENA (todas las tiendas).
...
begin
  ...
  metrics as (
    select
      ...
      st.name      as tienda,
      -- costo promedio unitario del genérico (ponderado por unidades, cadena)
      (select round(sum(sc.cost * sc.units) / nullif(sum(sc.units),0), 2)
         from stock_current sc where sc.generic_code = v.generic_code) as costo_prom,
      -- stock tienda / cadena
      coalesce((select sum(units) from stock_current sc
                 where sc.generic_code = v.generic_code and sc.store_id = v.store_id), 0) as stock_tienda,
      coalesce((select sum(units) from stock_current sc
                 where sc.generic_code = v.generic_code), 0) as stock_cadena,
      -- ventas de la ventana (tienda / cadena)
      coalesce((select sum(units) from sales_daily sd
                 where sd.generic_code = v.generic_code and sd.store_id = v.store_id
                   and sd.sale_date > current_date - p_window_days), 0) as venta_tienda,
      coalesce((select sum(units) from sales_daily sd
                 where sd.generic_code = v.generic_code
